  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0 -top-20 bg-soul-dark">
        ...
        <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[700px] h-[500px] bg-soul-purple/10 rounded-full blur-3xl hero-glow" />
      </div>

      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-20">
        <div className="text-center">
          {/* HERO LOGO — switches for light/dark */}
          <div className="mb-6 sm:mb-8 flex justify-center">
            <img
              src={isDark ? '/logo_black.png' : '/logo_clear.png'}
              alt="Soulcore.dev"
              className="h-32 sm:h-44 md:h-56 lg:h-64 w-auto hero-logo drop-shadow-[0_0_40px_rgba(139,92,246,0.3)]"
            />
          </div>

          {/* Badge */}
          <div className="mb-8">
            <span className="inline-flex items-center px-4 py-1.5 rounded-full bg-soul-purple/10 border border-soul-purple/20 text-soul-purple-light text-sm font-medium tracking-wide">
          ...
          <p className="text-base sm:text-lg md:text-xl text-gray-400 max-w-2xl mx-auto mb-12 leading-relaxed">
            {t.rich('subtitle', {
              strong: (chunks) => <strong className="text-soul-purple font-semibold">{chunks}</strong>
            })}
          </p>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-14">
            ...
            <Link
              href={`/${locale}/store`}
              className="w-full sm:w-auto px-10 py-4 bg-emerald-600/20 border-2 border-emerald-500/30 hover:bg-emerald-600/30 hover:border-emerald-500/50 rounded-xl font-bold text-lg text-emerald-400 transition-all duration-200 flex items-center justify-center group"
            >
          ...
          {/* Stats */}
          <div className="hero-stats grid grid-cols-3 gap-3 max-w-md mx-auto rounded-2xl p-4 bg-black/30 backdrop-blur-md">
            {stats.map((stat) => (
              <div key={stat.label} className="flex flex-col items-center">
                ...
                {stat.value && <span className="text-2xl sm:text-3xl font-bold text-soul-purple">{stat.value}</span>}
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2">
        <div className="scroll-indicator w-6 h-10 border-2 border-white/15 rounded-full flex justify-center pt-2">
          <div className="scroll-dot w-1.5 h-1.5 bg-white/60 rounded-full animate-bounce" />
        </div>
      </div>
    </section>
  )
